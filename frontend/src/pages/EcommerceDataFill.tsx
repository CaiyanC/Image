...
export default function EcommerceDataFill() {
  ...
  const effectiveParameters = mode === 'amazon' ? {} : parameters
  ...
  function reset(nextMode: Mode) { setMode(nextMode); setDraft(null); setPrecheck(null); setSelectedRole(null); setStep(1); setError('') }
  async function addFiles(files: File[], role?: string) {
    if (!files.length) return
    setLoading(true); setError(''); setPrecheck(null)
    try {
      const next = draft
      ...
      setDraft(updated)
      setPrecheck(await api.tools.ecommerceDataFill.precheckDraft(updated.id, effectiveParameters))
  ...
  function chooseSlot(role: string) {
    if (loading) return
    flushSync(() => { setSelectedRole(role); setStep(1) })
    slotInput.current?.click()
  }
  async function check() { if (!draft) { setError('请先上传 Excel 文件。'); return }; setLoading(true); setError(''); try { setPrecheck(await api.tools.ecommerceDataFill.precheckDraft(draft.id, effectiveParameters)); setStep(3) } catch (err) { setPrecheck(null); setError(err instanceof Error ? err.message : '预检查失败') } finally { setLoading(false) } }
  async function confirmRun() {
    if (!draft || !precheck?.can_run) return
    if (!window.confirm('请确认已关闭本次使用的 Excel 文件。确认后将开始填写。')) return
    setLoading(true); setError('')
    try { await api.tools.ecommerceDataFill.confirmDraft(draft.id, effectiveParameters); setStep(4); await refreshRuns() } catch (err) { setError(err instanceof Error ? err.message : '开始填写失败') } finally { setLoading(false) }
  }
  ...
  return <main className="mx-auto max-w-6xl px-4 pb-12 pt-8 md:px-6">
    ...
    <section className="glass mt-6 rounded-3xl p-6">
      ...
      {step === 2 && <>
        <h2 className="text-lg font-black text-apple-text">2. 确认填写条件</h2>
        {mode === 'amazon' ? <p className="mt-2 text-sm text-apple-gray-medium">亚马逊库存按上传的三份文件填写，无需设置周期和日期。</p> : <>
          <p className="mt-2 text-sm text-apple-gray-medium">这些值会按原工具逻辑写入本次结果。</p>
          <div className="mt-5 grid gap-3 md:grid-cols-2">
            {([['cycle_code','周次编码，例如 W27'], ['start_date','开始日期，YYYY-MM-DD'], ['end_date','结束日期，YYYY-MM-DD']] as const).map(([key, label]) => <input key={key} value={parameters[key]} placeholder={label} onChange={(event) => setParameters({ ...parameters, [key]: event.target.value })} className="rounded-xl border border-black/10 bg-white/60 px-4 py-3 text-sm" />)}
            <select value={parameters.cycle_type} onChange={(event) => setParameters({ ...parameters, cycle_type: event.target.value })} className="rounded-xl border border-black/10 bg-white/60 px-4 py-3 text-sm"><option>周</option><option>月</option></select>
          </div>
        </>}
        <button onClick={check} disabled={loading} className="mt-5 rounded-full bg-teal-700 px-5 py-2.5 text-sm font-bold text-white">运行预检查</button>
      </>}
...
function FileSlotTable({ mode, precheck, files, onChoose }: { mode: Mode; precheck: EcommercePrecheck | null; files: ToolRun['input_files']; onChoose: (role: string) => void }) {
  const slots: EcommercePrecheck['slots'] = precheck?.slots || workflowSlots[mode].map((slot) => ({ ...slot, recognized: false }))
  return <div className="mt-5 overflow-x-auto rounded-2xl border border-black/10"><table className="w-full text-left text-sm">
    <thead className="bg-slate-50 text-xs text-apple-gray-medium"><tr><th className="px-4 py-3">文件类型</th><th className="px-4 py-3">状态</th><th className="px-4 py-3">当前文件</th><th className="px-4 py-3">操作</th></tr></thead>
    <tbody>{slots.map((slot) => {
      const file = slot.file && files.find((item) => item.relative_path === slot.file?.relative_path)
      return <tr key={slot.role} className="border-t border-black/5">
        <td className="px-4 py-3 font-bold text-apple-text">{slot.label}{slot.required && <span className="ml-2 text-xs text-red-600">必需</span>}</td>
        <td className={`px-4 py-3 font-bold ${slot.recognized ? 'text-teal-700' : slot.required ? 'text-red-600' : 'text-amber-700'}`}>{slot.recognized ? '已导入' : slot.required ? '未导入' : '未导入（建议）'}</td>
        <td title={file?.display_name} className="max-w-52 break-all px-4 py-3 text-apple-gray-medium">{file?.display_name || '—'}</td>
        <td className="px-4 py-3"><button onClick={() => onChoose(slot.role)} className="rounded-full bg-teal-50 px-3 py-1.5 text-xs font-bold text-teal-700">{slot.recognized ? '替换文件' : '选择文件'}</button></td>
      </tr>
    })}</tbody>
  </table></div>
}
