...
- 代码改动后，在 `D:\CaiYan\Image-Generation-feature-v5-graphify` 依次执行 `graphify update . --no-cluster` 和 `graphify cluster-only . --no-label`，再检查受影响社区和关键路径。两条命令只处理代码图谱，不需要 API；第二条会同步刷新报告和 HTML。
