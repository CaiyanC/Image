...
set "RELEASE_ROOT="
set "RELEASE_COMMIT="
for /f "tokens=1,* delims==" %%a in ('powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0deploy\scripts\prepare_production_release.ps1" -SourceRepo "%RUNTIME_ROOT%" ^| findstr /B "RELEASE_"') do set "%%a=%%b"
...
powershell -NoProfile -ExecutionPolicy Bypass -File "%RELEASE_ROOT%\deploy\scripts\service_control_windows.ps1" -Action StopAll -RepoRoot "%RELEASE_ROOT%" -RuntimeRoot "%RUNTIME_ROOT%" -DependencyRoot "%RUNTIME_ROOT%" -EnvFile "%~dp0backend\.env" -LogPath "%~dp0logs\watchdog.log" -ExpectedCommit "%RELEASE_COMMIT%" -AllowLegacySharedProcess
if errorlevel 1 exit /b %errorlevel%
powershell -NoProfile -ExecutionPolicy Bypass -File "%RELEASE_ROOT%\deploy\scripts\service_control_windows.ps1" -Action All -RepoRoot "%RELEASE_ROOT%" -RuntimeRoot "%RUNTIME_ROOT%" -DependencyRoot "%RUNTIME_ROOT%" -EnvFile "%~dp0backend\.env" -LogPath "%~dp0logs\watchdog.log" -ExpectedCommit "%RELEASE_COMMIT%"
if errorlevel 1 exit /b %errorlevel%
...
powershell -NoProfile -ExecutionPolicy Bypass -File "%RELEASE_ROOT%\deploy\scripts\service_control_windows.ps1" -Action StopAll -RepoRoot "%RELEASE_ROOT%" -RuntimeRoot "%RUNTIME_ROOT%" -DependencyRoot "%RUNTIME_ROOT%" -EnvFile "%~dp0backend\.env" -LogPath "%~dp0logs\watchdog.log" -ExpectedCommit "%RELEASE_COMMIT%"
if errorlevel 1 exit /b %errorlevel%
powershell -NoProfile -ExecutionPolicy Bypass -File "%RELEASE_ROOT%\deploy\scripts\service_control_windows.ps1" -Action All -RepoRoot "%RELEASE_ROOT%" -RuntimeRoot "%RUNTIME_ROOT%" -DependencyRoot "%RUNTIME_ROOT%" -EnvFile "%~dp0backend\.env" -LogPath "%~dp0logs\watchdog.log" -ExpectedCommit "%RELEASE_COMMIT%"
exit /b %errorlevel%
